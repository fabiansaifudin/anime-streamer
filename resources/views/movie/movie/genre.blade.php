            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="bgc-white bd bdrs-3 p-20 mB-20">
                        <h4 class="c-grey-900 mB-20">Kategori Movie</h4>
                        ...
                        <div class="row mt-3">
                            <div class="mb-3 col-md-4">
                                <select id="inputState" class="form-control">
                                    <option disabled selected>All Movie Genre</option>
                        ...
                        <table id="dataTableGenre" class="table table-striped mT-15 table-hover" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr id="sid{{ $data->id }}">
                                    <th scope="row">{{ $data->judul_movie }}</th>
                                    <td>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addModalAnimeForm" action="{{ route('saveMovieGenre') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="post_id" id="post_id">
                    <div class="mb-3 row">
                        <label for="genreMovie" class="form-label col-sm-3 col-form-label">Genre Movie</label>
                        <div class="col-sm-9">
                            <select class="form-select" aria-label="Default select example" name="genre[]" id="genreMovie" multiple required>
                                <option disabled selected>Pilih Genre Movie</option>
                                @php
                                use App\Anime\Genre;
                                $genre = Genre::get();
                                @endphp
...
@endif
<script>
    $('#dataTableGenre').DataTable({paging: false});
    ...
    function show() {
        ...
        $('tbody').html('');
        $.get("{{ route('findMovieGenre', '') }}"+to.value, function (params) {

            $.get("{{ route('whereMovieGenre', '') }}"+to.value, function (p) {
                for (let i = 0; i < params.genre.length; i++) {
                    $('tbody').append('<tr><th scope="row">'+params.judul_movie+'</th><td><div class="col col-genre"><div class="genre"><span class="text-genre">'+params.genre[i].genre+'</span></div></div></td><td><a href="javascript:void(0);" data-id="'+p[i].id+'" class="action c-orange-500 swal-delete"><form action="{{ route("deleteMovieGenre", "") }}'+p[i].id+'" id="delete'+p[i].id+'" method="POST">@csrf @method("delete")<i class="ti-trash" data-toggle="tooltip" title="Delete"></i></form></a></td></tr>');
                }
