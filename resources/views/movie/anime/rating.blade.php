                ...
                <div class="col-md-12">
                    <div class="bgc-white bd bdrs-3 p-20 mB-20">
                        ...
                        <table id="dataTableAnime" class="table table-striped mT-15 table-hover" cellspacing="0">
                            ...
                            <tbody>
                                @foreach ($anime as $no => $data)
                                <tr id="sid{{ $data->id }}">
                                    <th scope="row">{{ $anime->firstItem()+$no }}</th>
                                    <td>{{ $data->judul_anime }}</td>
                                    <td>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="statusUpdateForm" action="{{ route('saveRating') }}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="id" id="id">
                    <div class="mb-3 row">
                        <label for="updateJudelAnime" class="form-label col-sm-3 col-form-label">Judul Anime</label>
                        <div class="col-sm-9">
                            <label id="updateJudelAnime" class="form-label col-form-label" style="font-style: normal; font-weight: bold;">Judul table anime</label>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label for="ratingAnime" class="form-label col-sm-3 col-form-label">Rating Anime</label>
                        <div class="col-sm-9">
                            <input type="range" style="width: 80%" min="0" max="10" value="0" step="0.01" name="rating_anime" id="ratingAnime" />
...
<script>
    function editStatus(id) {
        $.get("{{ route('findRating', '') }}"+id, function(rat) {
            $('#id').val(rat.id);
            $('#updateJudelAnime').html(rat.judul_anime)
            $('#ratingAnime').val(rat.rating.rating);
            $('#rating').text(rat.rating.rating);
            $("#editAnimeModal").modal("toggle");
        });
        // $.get("{{ route('findRating', '') }}"+id, function(r) {
        //     $("#id").val(r.id);
        //     $('#updateJudelAnime').html(r.judul_anime);
        //     if (r.rating.length == 0) {
        //         $('#ratingAnime').val(0);
        //         $('#rating').text(0);
        //     } else {
        //         $('#ratingAnime').val(r.rating[0].rating);
        //         $('#rating').text(r.rating[0].rating);
        //     }
        //     $("#editAnimeModal").modal("toggle");
        // });
    }
